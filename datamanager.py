...
def download_data(fund: str = 'MAC', market: str = 'TEFAS', start: str = '2025-01-01'):
    ...
    if market == "TEFAS":    
        while days_between(start, end_date) > 90:
            ...
            data = tefas_data.fetch(start=start, end=temporary_end_date, name=fund, columns=["code", "date", "price", "stock", "number_of_investors"])
            ...
            if days_between(start, end_date) > 1:
                data = tefas_data.fetch(start=start, end=end_date, name=fund, columns=["code", "date", "price", "stock", "number_of_investors"])
                data_list.append(data)
